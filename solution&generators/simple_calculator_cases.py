...
for case in manual_cases:
    with open(f"inputs/simple_calculator/{case_num}.txt", "w") as f:
        f.write(case)
    
    output = str(calculator(case))
    with open(f"outputs/simple_calculator/{case_num}.txt", "w") as f:
        f.write(output)
    case_num += 1
...
for i in range(2):
    case = generate_test_case(max_N // random.randint(2, 10) + random.randint(1000, 5000), 99999)
    output = str(calculator(case))
    with open(f"inputs/simple_calculator/{case_num}.txt", "w") as f:
        f.write(case)
    with open(f"outputs/simple_calculator/{case_num}.txt", "w") as f:
